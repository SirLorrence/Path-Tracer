...
void Camera::Render(const RenderObject &world) {
  ...
  std::ofstream f_out{image_name ,std::ios::binary};
    // PNG writer
  TinyPngOut png_image{static_cast<std::uint32_t>(img_width),static_cast<std::uint32_t>(img_height),f_out};
  // data() creates a pointer to the array not to the type of vector
  png_image.write(pixel_array.data(), static_cast<std::size_t>(img_width*img_height));
}
